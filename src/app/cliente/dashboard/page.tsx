...
export default function Dashboard() {
      ...
      <div className="bg-white shadow-md rounded-2xl p-6 w-full max-w-sm space-y-6">
        ...
        <div className="divide-y divide-gray-200 text-gray-700">
          ...
          <div className="flex justify-between py-2">
            ...
            <span>{cliente?.dataNascimento ? new Date(cliente.dataNascimento).toLocaleDateString('pt-BR') : "Não informado"}</span>
          </div>
        </div>
  ...
  const Cupons = () => {
    ...
    if (!cartoes || cartoes.length === 0) return <div className="p-4 text-center text-gray-500">Nenhum cupom disponível.</div>;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {listaExibir.map(cupom => (
            <div key={cupom.codigo} className="p-3 bg-white shadow rounded-lg flex flex-col gap-1">
              ...
              {cupom.dataResgate && (
                <p className="text-xs text-green-600">Resgatado: {new Date(cupom.dataResgate).toLocaleDateString('pt-BR')}</p>
              )}
            </div>
          ))}
